...
import { API } from "../App"
import { Scan, Upload, Loader2, AlertCircle, CheckCircle, Activity } from "lucide-react"
...
export default function SkinAnalysis() {
  ...
  const analyze = async () => {
    ...
    fd.append("file", file)
    try {
      const res = await axios.post(`${API}/analyze/skin`, fd)
      setResult(res.data)
  ...
  const color = isHigh ? "#FF3366" : result?.risk_level === "MEDIUM" ? "#FFB340" : "#00FFB3"

  return (
    <div className="page grid-bg">
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
        <Activity size={14} color="#8B5CF6" />
        <span style={{ fontSize: "11px", color: "#8B5CF6", fontFamily: "var(--font-m)", letterSpacing: "2px" }}>
          SKIN DISEASE DETECTION
        </span>
      </div>
      <h1 className="section-title">Dermatology Analysis</h1>
      <p className="section-sub">Upload a skin lesion image to detect melanoma, skin cancer, and other conditions</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div
            ...
            style={{
              ...
              borderColor: drag ? "#8B5CF6" : preview ? "#8B5CF644" : "var(--border)",
              borderStyle: "dashed", borderWidth: "1px",
              transition: "all 0.2s", minHeight: "260px",
              display: "flex", flexDirection: "column",
              alignItems: "center", justifyContent: "center",
              background: drag ? "rgba(139,92,246,0.04)" : "rgba(8,14,28,0.9)"
            }}>
            <input ref={inputRef} type="file" accept="image/*" style={{ display: "none" }}
              onChange={e => e.target.files?.[0] && handleFile(e.target.files[0])} />
            {preview ? (
              <img src={preview} alt="Skin" style={{ width: "100%", maxHeight: "220px", objectFit: "cover", borderRadius: "10px" }} />
            ) : (
              <>
                <div style={{
                ...
                <div style={{ fontSize: "15px", fontWeight: 600, color: "var(--text-1)", marginBottom: "6px" }}>Drop skin image here</div>
                <div style={{ fontSize: "12px", color: "var(--text-3)" }}>or click to browse · JPG, PNG</div>
              </>
            )}
          </div>
          ...
          <button className="btn-primary" onClick={analyze} disabled={!file || loading}
            style={{ background: !file || loading ? undefined : "linear-gradient(135deg, #8B5CF6, #5B21B6)" }}>
            {loading
            ...
            <div className="glass fade-in" style={{ padding: "24px", borderColor: `${color}30` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                ...
                <span className="tag" style={{
                  background: isHigh ? "var(--red-dim)" : "var(--violet-dim)",
              ...
              <div style={{ marginBottom: "20px" }}>
                <div style={{ fontSize: "28px", fontWeight: 700, fontFamily: "var(--font-d)", color, letterSpacing: "-0.5px", textTransform: "capitalize" }}>
                  ...
                  .map(([disease, prob]: any) => (
                    <div key={disease}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                        <span style={{ fontSize: "12px", color: "var(--text-2)", textTransform: "capitalize" }}>
                          {disease.replace(/_/g, " ")}
                        </span>
                      ...
                      <div className="progress-track">
                        <div className="progress-fill" style={{
                          width: `${prob}%`,
                          background: disease === result.disease ? color : "rgba(255,255,255,0.08)"
                        }} />
                      </div>
                    </div>
                  ))}
