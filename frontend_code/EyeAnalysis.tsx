...
export default function EyeAnalysis() {
  ...
  const analyze = async () => {
    ...
    try {
      const res = await axios.post(`${API}/analyze/eye`, formData)
      setResult(res.data)
    } catch (e) {
  ...
  return (
    <div>
      ...
      <p style={{ color: "#444", marginBottom: "32px", fontSize: "14px" }}>
        Detect jaundice, anemia, cataract from eye fundus images
      </p>
        ...
        <div>
          <div onClick={() => inputRef.current?.click()}
            ...
            {preview ? (
              ...
              <>
                <Eye size={40} color="#333" style={{ marginBottom: "12px" }} />
                <div style={{ color: "#555", fontSize: "14px" }}>Upload eye image</div>
              </>
            )}
          </div>
          <button onClick={analyze} disabled={!file || loading}
            ...
            {loading ? <><Loader2 size={16} style={{ animation: "spin 1s linear infinite" }} /> Analyzing...</> : <><Upload size={16} /> Analyze Eye</>}
          </button>
        </div>
